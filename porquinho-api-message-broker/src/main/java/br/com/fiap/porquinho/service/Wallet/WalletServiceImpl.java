package br.com.fiap.porquinho.service.Wallet;
...
public class WalletServiceImpl implements WalletService<Wallet, Long> {
    ...
    public Wallet create(Wallet wallet) {
        if (wallet.getName() == null || wallet.getName().isBlank()) {
            throw new FieldValidationException("name", "O nome da carteira não pode estar em branco.");
        }
        if (wallet.getUser() == null) {
            throw new FieldValidationException("user", "A carteira deve estar vinculada a um usuário.");
        }
